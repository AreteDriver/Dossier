...
def ingest_file(filepath: str, source: str = "", date: str = "") -> dict:
    ...
    if not raw_text or len(raw_text.strip()) < 20:
        return {"success": False, "message": f"No text extracted from {filepath.name} (method: {extraction['method']})"}

    pages = extraction["pages"]
    print(f"[INGEST] Extracted {len(raw_text)} chars, {pages} pages via {extraction['method']}")

    # ─── Step 3: NER + Classification ───
    print(f"[INGEST] Running NER...")
    ...
    return {"success": True, "document_id": doc_id, "message": "Ingested successfully", "stats": stats}


def ingest_directory(dirpath: str, source: str = "") -> list[dict]:
    """Ingest all supported files in a directory."""
    dirpath = Path(dirpath)
    supported = {".pdf", ".txt", ".md", ".html", ".htm", ".png", ".jpg", ".jpeg", ".tiff", ".tif", ".bmp"}
    results = []
