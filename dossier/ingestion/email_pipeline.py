...
from dossier.ingestion.email_parser import parse_email_file
...
def ingest_email_file(filepath: str, source: str = "", corpus: str = "") -> list[dict]:
    ...
    for email_data in emails:
        result = _ingest_single_email(
            email_data, source=source, corpus=corpus, origin_file=str(path)
        )
...
def ingest_email_directory(dirpath: str, source: str = "", corpus: str = "") -> dict:
    ...
    print(
        f"[EMAIL-INGEST] Complete: {success} ingested, {failed} failed out of {len(all_results)} emails"
    )
    return {
        "ingested": success,
        "failed": failed,
...
def _ingest_single_email(
    email_data: dict, source: str = "", corpus: str = "", origin_file: str = ""
) -> dict:
